...
function generateMockStockData(symbol: string) {
  // Simple seed based on symbol char codes
  const upperSymbol = symbol.toUpperCase();
  const seed = upperSymbol.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  
  // Deterministic baseline calculations
  const basePrice = 50 + (seed % 1950); // baseline price between ₹50 and ₹2000
  const roe = 10 + (seed % 20); // ROE between 10% and 30%
  const pe = 12 + (seed % 35); // PE between 12 and 47
  const eps = Number((basePrice / pe).toFixed(2));
  const changePercent = -3 + ((seed % 70) / 10); // daily change between -3% and +4%
  ...
  const seedName = DEFAULT_SEEDS.find(s => s.symbol.toUpperCase() === upperSymbol)?.name 
    || `${upperSymbol.replace('.NS', '').replace('.BO', '')} Industries Ltd.`;
  ...
  const profile = {
    ...
    website: `https://www.${seedName.toLowerCase().replace(/[^a-z0-9]/g, '')}.com`,
    city: ['Mumbai', 'Bengaluru', 'Chennai', 'New Delhi', 'Gurugram', 'Hyderabad', 'Pune', 'Kolkata'][seed % 8],
    summary: `${seedName} operates as a premier enterprise in the ${industry} sector, providing high-quality solutions and services across domestic and international markets. The company prides itself on top-tier engineering standards, exceptional operational efficiency, and customer-first methodologies. With over ${250 + (seed % 18000)} professionals across state-of-the-art administrative facilities, the firm continues to build long-term value for shareholders and institutional partners globally.`,
    officers: [
      { name: 'Dr. Ramesh Kumar', title: 'Managing Director & CEO', age: 52, pay: 12000000 },
      { name: 'Sanjay Sharma', title: 'Whole-time Director & CFO', age: 48, pay: 9500000 },
      { name: 'Anjali Gupta', title: 'Non-Executive Independent Chairperson', age: 58, pay: 3000000 },
      { name: 'Vikram Singh', title: 'VP - Operations & Engineering', age: 45, pay: 7200000 },
      { name: 'Neha Patel', title: 'Company Secretary & Compliance Officer', age: 39, pay: 4500000 }
    ]
  };
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const decay = Math.pow(0.9, i); // decay backwards in time
  ...
  const chartData = [];
  let currentPrice = basePrice * 0.85; // start lower 1 year ago
  const now = new Date();
  for (let i = 60; i >= 0; i--) {
    const date = new Date(now);
    date.setDate(now.getDate() - i * 6); // roughly spaced out over 1 year
    const randChange = -1.5 + ((seed * i) % 310) / 100; // deterministic random walk step
    currentPrice = currentPrice * (1 + randChange / 100);
    
    // Bind price to realistic high/low boundaries
  ...
  const news = [
    { title: `${seedName} announces robust corporate actions and expansion of key operational verticals`, publisher: 'Business Standard', link: '#', date: '30 May 2026' },
    { title: `Analysts upgrade stock recommendation on ${upperSymbol} citing resilient margins`, publisher: 'Economic Times', link: '#', date: '29 May 2026' },
    { title: `Institutional Accumulation spotted in ${upperSymbol} as promoter buying builds momentum`, publisher: 'LiveMint', link: '#', date: '25 May 2026' },
    { title: `${seedName} clinches massive order worth ₹${seed % 850} Crores in industrial segment`, publisher: 'CNBC-TV18', link: '#', date: '18 May 2026' }
  ];
...
export async function GET(
  ...
  try {
    ...
    if (summaryRes.status === 'rejected') {
      console.warn(`[YahooFinance API Error] Failed to fetch data for ${symbol}. Using high-fidelity mock generator fallback.`);
